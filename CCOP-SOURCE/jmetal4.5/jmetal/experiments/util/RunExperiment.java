	...
	public void run() {
		...
		while(!finished){
			...
			if(!finished && problemData != null){
				...
				COPConfigurator.computeTestSetDistance(resultFront,algorithmNameList_[alg], runs);
				COPConfigurator.seed = COPConfigurator.seed + 1;
				COPConfigurator.ran = new Random(COPConfigurator.seed);
				System.out.println("Seed "+COPConfigurator.seed+"\n \n");
				COPConfigurator.writeSolutionsToFile(resultFront,COPConfigurator.baseDir+"/MyStandardStudyCOP" + "/VAR_"+runs+"_"+algorithmNameList_[alg]); // WRITE INTERNAL SOLUTION TO FILE
