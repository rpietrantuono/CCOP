...
public class csMOPSO extends Algorithm {
  ...
  */    
  ...
  public void initParams(){
    ...
    valuesOfBest_  = new double[particlesSize_][causalOptimization.COPConfigurator.MAX_sources + causalOptimization.COPConfigurator.MAX_targets];
    ...
    speed_ = new double[particlesSize_][causalOptimization.COPConfigurator.MAX_sources + causalOptimization.COPConfigurator.MAX_targets]; // Target size is always = 1 in the current implementation of ABLA
  } // initParams
...
private void computeSpeed() throws JMException{        
    ...
    for (int i = 0; i < particlesSize_; i++){
      ...
      */     
      for (int j = 0; j < causalOptimization.COPConfigurator.MAX_sources; j++){                                     
          //Computing the velocity of this particle
      ...
      for (int j = causalOptimization.COPConfigurator.MAX_sources; j < causalOptimization.COPConfigurator.MAX_targets+ causalOptimization.COPConfigurator.MAX_sources; j++){                                     
          //Computing the velocity of this particle
          speed_[i][j] = W  * speed_[i][j] +
                     C1 * r1 * ((Targets)(best_[i].getDecisionVariables()[1]))._doubleValueList.get(j-causalOptimization.COPConfigurator.MAX_sources) - 
                       			((Targets)(particles_.get(i).getDecisionVariables()[1]))._doubleValueList.get(j-causalOptimization.COPConfigurator.MAX_sources) +
                     C2 * r2 *((Targets)(bestGlobal.getDecisionVariables()[1]))._doubleValueList.get(j-causalOptimization.COPConfigurator.MAX_sources) - 
                     ((Targets)(particles_.get(i).getDecisionVariables()[1]))._doubleValueList.get(j-causalOptimization.COPConfigurator.MAX_sources); 
        }
  ...
  private void computeNewPositions() throws JMException{
	 ...
	 double upperSourceLimit = ((double)COPConfigurator.possibleSources.size())/(COPConfigurator.possibleSources.size()+COPConfigurator.possibleTargets.size());
	 double lowerTargetLimit = ((double)COPConfigurator.possibleSources.size())/(COPConfigurator.possibleSources.size()+COPConfigurator.possibleTargets.size())+ 0.5/(COPConfigurator.possibleTargets.size()+COPConfigurator.possibleTargets.size());
 ...
 /*     for (int var = 0; var < particle.length; var++){
    ...
    for (int j = 0; j < causalOptimization.COPConfigurator.MAX_sources; j++){ //REPLACE WITH "NUMBER OF GENES"
  	  ...
  	  if (((Sources)(particles_.get(i).getDecisionVariables()[0]))._doubleValueList.get(j) > upperSourceLimit){
  		  ...
  		  speed_[i][j] = speed_[i][j] * -1.0;
  		((Sources)(particles_.get(i).getDecisionVariables()[0]))._sourcesFullList.set(j, String.valueOf(COPConfigurator.possibleSources.size()));
  		  continue;
  	  }
  	  try {
    ...
    for (int j = 0; j < causalOptimization.COPConfigurator.MAX_targets; j++){
    	String stringRepresentation;
    	...
    	if (((Targets)(particles_.get(i).getDecisionVariables()[1]))._doubleValueList.get(j) < lowerTargetLimit){
  		  ...
  		  speed_[i][j+causalOptimization.COPConfigurator.MAX_sources] = speed_[i][j+causalOptimization.COPConfigurator.MAX_sources] * -1.0;
  	  ...
  	  if (((Targets)(particles_.get(i).getDecisionVariables()[1]))._doubleValueList.get(j) > upperTargetLimit){
    	  ...
    	  ((Targets)(particles_.get(i).getDecisionVariables()[1]))._targetsFullList.set(j, String.valueOf(COPConfigurator.possibleSources.size()+COPConfigurator.possibleTargets.size()));
  		  speed_[i][j+causalOptimization.COPConfigurator.MAX_sources] = speed_[i][j+causalOptimization.COPConfigurator.MAX_sources] * -1.0;
  		  continue;
  	  }
  	  try {
  ...
  */  
  public SolutionSet execute() throws JMException, ClassNotFoundException {
    ...
    for (int i = 0; i < particlesSize_; i++) {
    	for (int j = 0; j < causalOptimization.COPConfigurator.MAX_sources + causalOptimization.COPConfigurator.MAX_targets;  j++)  //problem_.getNumberOfVariables(); j++) { //CoP has two variables: source and target. In teh current implementation, "Target" size = 1.  
    	  speed_[i][j] = 0.0;
    }
