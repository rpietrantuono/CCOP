  ...
  */
  public void doMutation(double probability, Solution solution) throws JMException {  
	 ...
	 double upperTargetLimit = 1;
	  ...
	  for (int j = 0; j < causalOptimization.COPConfigurator.MAX_sources; j++){ //REPLACE WITH "NUMBER OF GENES"
		  if (PseudoRandom.randDouble() < probability) {
	  	  ...
	  	  if (tmp > upperSourceLimit){
	  		  ...
	  		  ((Sources)(solution.getDecisionVariables()[0]))._sourcesFullList.set(j, String.valueOf(COPConfigurator.possibleSources.size()));
	  		  continue;
	  	  }
	  	  try {
	  ...
	  for (int j = 0; j < causalOptimization.COPConfigurator.MAX_targets; j++){ //REPLACE WITH "NUMBER OF GENES"
		  if (PseudoRandom.randDouble() < probability) {
			  double rand = PseudoRandom.randDouble();
			  double tmp = (rand - 0.5)*perturbation_.doubleValue();
			  String stringRepresentation; 
		  	  ...
		  	  if (tmp > upperTargetLimit){
		  		...
		  		((Targets)(solution.getDecisionVariables()[1]))._targetsFullList.set(j, String.valueOf(COPConfigurator.possibleSources.size()+COPConfigurator.possibleTargets.size()));
		  		  continue;
		  	  }
		  	  try {
				stringRepresentation = util.SolutionUtils.getStringRepresentation(tmp);
	  ...
	  Iterator it1 = hs1.iterator();
		...
		*/
	    while (((Sources)(solution.getDecisionVariables()[0]))._sourcesList.size()<COPConfigurator.MIN_sources) {
	    	String sourceToAdd; 
	    	do {
	    		sourceToAdd = COPConfigurator.possibleSources.get(COPConfigurator.ran.nextInt(COPConfigurator.possibleSources.size()));
	    	}
