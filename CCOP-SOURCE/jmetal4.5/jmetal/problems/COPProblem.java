...
public class COPProblem extends Problem {
	...
	public COPProblem(String solutionType){
	  ...
	  */        
		  ...
		  if (!COPConfigurator.technique.equals("COP")) {
			  try {
		  ...
		  try {
			  ...
			  plausibility = COPConfigurator.plaus.rootPlausibilityEvaluation(solution, true); 
			  //plausibility = Main.plaus.logarithmicPlausibilityEvaluation(solution, true);
			  similarities = COPConfigurator.sim.computeKBSimilarity(solution);
		  }catch (Exception e) { e.printStackTrace();	} 
		  double total_sim =0; 
