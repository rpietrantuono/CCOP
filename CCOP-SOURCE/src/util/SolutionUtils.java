...
public static double getDoubleRepresentation(String s) throws Exception {
	...
	double maxSolutionSize = COPConfigurator.possibleSources.size() + COPConfigurator.possibleTargets.size();
...
public static String getStringRepresentation(Double d) throws Exception {
	...
	int maxSolutionSize = COPConfigurator.possibleSources.size() + COPConfigurator.possibleTargets.size();
...
public static Solution createSolutionFromString(ArrayList<String> solution_string) {
	...
	for (int i=0; i<solution_string.size(); i++ ) { 
		if (Integer.parseInt(solution_string.get(i)) <= COPConfigurator.targetLimitNumber)
	...
	return new Solution(COPConfigurator.myProblemString,variables); 
}
...
public static void writeSolutionVariablesToFile(String filename, Solution indiv) {
	...
	for(int j=0; j< sources.size();j++){
		...
		line_names = line_names + COPConfigurator.getNameFromKey(sources.get(j)) + " ";
	}//end for sources
		
	for(int j=0; j< targets.size();j++){
		line = line + targets.get(j) + " "; //targets
		line_names = line_names + COPConfigurator.getNameFromKey(targets.get(j)) + " ";
		}
...
public static void writePopulationStatisticsToFile(String filename, SolutionSet population) {
	...
	line = averagePlausibility+", " +averageNovelty+", "+medianPlausibility+", "+medianNovelty+", "+bestPlausibility+", "+bestNovelty+", "+worstPlausibility+", "+worstNovelty;
	pw.println(line);
	pw.flush();
	pw.close();		
